...
<script>
  ...
  function buildDynamicFallbackResult(town, lensId, mode) {
    ...
    // POKA-YOKE SAFE REVIEW PICKER: Prevent infinite loop when reviewPool.length < 3
    const safeReviewPool = (Array.isArray(reg.reviews) && reg.reviews.length >= 3)
      ? reg.reviews
      : [
          ...(reg.reviews || []),
          { reviewer: "ParishVisitor", rating: 4, text: "Historic market square atmosphere and friendly pub hearth." },
          { reviewer: "LocalShopper", rating: 3, text: "Overcast weather but excellent local bakery." },
          { reviewer: "CivicAuditor", rating: 2, text: "Slightly disoriented by parish noticeboard guidelines." }
        ];
